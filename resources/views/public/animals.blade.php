@extends('layouts.app')

@section('title', 'Quero Adotar - SAAU')

@section('header-content')
    <h1 class="page-header-title">QUERO ADOTAR</h1>
@endsection

@section('content')
<div class="container my-5">
    <div class="row g-4">
        
        {{-- =========================
             COLUNA ESQUERDA: FILTROS
             ========================= --}}
        <div class="col-lg-3 mb-4">
            <div class="card border-0 shadow-sm" style="border-radius: 15px; overflow: hidden;">
                <div class="card-header bg-white border-bottom-0 pt-4 px-4">
                    <h5 class="fw-bold text-primary mb-0">
                        <i class="fas fa-filter text-warning me-2"></i> Filtrar Busca
                    </h5>
                </div>
                <div class="card-body p-4">
                    <form method="GET" action="{{ route('animals') }}">
                        
                        <div class="mb-3">
                            <label class="form-label small fw-bold text-muted text-uppercase">Espécie</label>
                            <select name="species" class="form-select rounded-pill bg-light border-0 text-dark">
                                <option value="">Todas</option>
                                <option value="cão"  {{ request('species') == 'cão' ? 'selected' : '' }}>Cão</option>
                                <option value="gato" {{ request('species') == 'gato' ? 'selected' : '' }}>Gato</option>
                            </select>
                        </div>

                        <div class="mb-3">
                            <label class="form-label small fw-bold text-muted text-uppercase">Sexo</label>
                            <select name="gender" class="form-select rounded-pill bg-light border-0 text-dark">
                                <option value="">Todos</option>
                                <option value="macho" {{ request('gender') == 'macho' ? 'selected' : '' }}>Macho</option>
                                <option value="fêmea" {{ request('gender') == 'fêmea' ? 'selected' : '' }}>Fêmea</option>
                            </select>
                        </div>

                        <div class="mb-3">
                            <label class="form-label small fw-bold text-muted text-uppercase">Porte</label>
                            <select name="size" class="form-select rounded-pill bg-light border-0 text-dark">
                                <option value="">Todos</option>
                                <option value="pequeno" {{ request('size') == 'pequeno' ? 'selected' : '' }}>Pequeno</option>
                                <option value="médio"   {{ request('size') == 'médio' ? 'selected' : '' }}>Médio</option>
                                <option value="grande"  {{ request('size') == 'grande' ? 'selected' : '' }}>Grande</option>
                            </select>
                        </div>

                        <hr class="my-4 opacity-25">

                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-primary rounded-pill fw-bold py-2 shadow-sm">
                                APLICAR FILTROS
                            </button>
                            <a href="{{ route('animals') }}" class="btn btn-outline-secondary rounded-pill fw-bold py-2">
                                LIMPAR
                            </a>
                        </div>
                    </form>
                </div>
            </div>
        </div>

        {{-- =========================
             COLUNA DIREITA: GRID DE ANIMAIS
             ========================= --}}
        <div class="col-lg-9">
            <div class="row g-4">
                @forelse($animals as $animal)
                <div class="col-md-6 col-xl-4 d-flex align-items-stretch">
                    
                    <div class="card h-100 w-100 border-0 shadow-sm card-animal hover-lift">
                        
                        {{-- FOTO DO ANIMAL --}}
                        <div class="position-relative overflow-hidden animal-img-container">
                            @if($animal->photo_url)
                                <img src="{{ $animal->photo_url }}" class="card-img-top" alt="{{ $animal->name }}">
                            @else
                                <div class="card-img-top bg-light d-flex align-items-center justify-content-center">
                                    <i class="fas fa-paw fa-3x text-muted opacity-25"></i>
                                </div>
                            @endif

                            {{-- Badge de Especial --}}
                            @if($animal->special_needs)
                                <span class="badge bg-warning text-dark position-absolute top-0 end-0 m-3 shadow-sm px-3 py-2 rounded-pill fw-bold">
                                    <i class="fas fa-star me-1"></i> Especial
                                </span>
                            @endif
                            
                            {{-- Overlay de Hover (Efeito visual) --}}
                            <div class="hover-overlay"></div>
                        </div>

                        {{-- CORPO DO CARD --}}
                        <div class="card-body p-0 d-flex flex-column text-center">
                            
                            {{-- Nome --}}
                            <div class="pt-4 pb-2 px-3">
                                <h4 class="card-title fw-bold text-primary mb-1 text-uppercase" style="letter-spacing: 1px;">
                                    {{ $animal->name }}
                                </h4>
                                <small class="text-muted fw-bold">{{ ucfirst($animal->species) }}</small>
                            </div>

                            {{-- Grid de Informações (Ícones) --}}
                            <div class="row g-0 py-3 mt-2 border-top border-bottom border-light-subtle w-100">
                                
                                {{-- Coluna 1: Sexo --}}
                                <div class="col-4 border-end border-light-subtle">
                                    @php $genderIcon = $animal->gender === 'macho' ? 'mars' : 'venus'; @endphp
                                    <div class="text-warning fs-5 mb-1"><i class="fas fa-{{ $genderIcon }}"></i></div>
                                    <span class="small text-muted fw-bold text-uppercase">{{ $animal->gender }}</span>
                                </div>

                                {{-- Coluna 2: Idade --}}
                                <div class="col-4 border-end border-light-subtle">
                                    <div class="text-warning fs-5 mb-1"><i class="far fa-calendar-alt"></i></div>
                                    <span class="small text-muted fw-bold text-uppercase">{{ ucfirst($animal->age ?? '-') }}</span>
                                </div>

                                {{-- Coluna 3: Porte --}}
                                <div class="col-4">
                                    <div class="text-warning fs-5 mb-1"><i class="fas fa-compress-arrows-alt"></i></div>
                                    <span class="small text-muted fw-bold text-uppercase">{{ ucfirst($animal->size ?? '-') }}</span>
                                </div>
                            </div>

                            {{-- Botão de Ação --}}
                            <div class="p-4 mt-auto">
                                <a href="{{ route('animal.show', $animal->id) }}" class="btn btn-outline-primary w-100 rounded-pill fw-bold py-2 shadow-sm stretched-link transition-btn">
                                    QUERO ADOTAR <i class="fas fa-heart ms-1"></i>
                                </a>
                            </div>

                        </div>
                    </div>

                </div>
                @empty
                <div class="col-12">
                    <div class="text-center py-5 bg-white rounded-4 shadow-sm">
                        <div class="mb-3 text-muted opacity-50">
                            <i class="fas fa-search fa-4x"></i>
                        </div>
                        <h4 class="text-dark fw-bold">Nenhum amigo encontrado</h4>
                        <p class="text-muted">Tente mudar os filtros para encontrar seu pet ideal.</p>
                        <a href="{{ route('animals') }}" class="btn btn-primary rounded-pill mt-2 px-4">
                            Limpar Filtros
                        </a>
                    </div>
                </div>
                @endforelse
            </div>

            <div class="mt-5 d-flex justify-content-center">
                {{ $animals->withQueryString()->links() }}
            </div>
        </div>
    </div>
</div>

<style>
    /* ESTILIZAÇÃO ESPECÍFICA PARA ORGANIZAÇÃO DOS CARDS */
    
    .card-animal {
        border-radius: 20px;
        transition: transform 0.3s ease, box-shadow 0.3s ease;
        background: #fff;
        overflow: hidden;
    }

    .animal-img-container {
        height: 260px; /* Altura fixa para todas as imagens */
        background-color: #f8f9fa;
    }

    .card-img-top {
        width: 100%;
        height: 100%;
        object-fit: cover; /* Garante que a foto preencha sem distorcer */
        transition: transform 0.5s ease;
    }

    /* Efeito Hover no Card Completo */
    .hover-lift:hover {
        transform: translateY(-5px);
        box-shadow: 0 15px 30px rgba(0, 86, 179, 0.15) !important; /* Sombra Azulada */
    }

    .hover-lift:hover .card-img-top {
        transform: scale(1.05);
    }

    /* Ajuste fino no botão */
    .transition-btn {
        transition: all 0.3s ease;
        border-width: 2px;
    }
    
    .hover-lift:hover .transition-btn {
        background-color: var(--saau-blue-primary);
        color: #fff;
        border-color: var(--saau-blue-primary);
    }
</style>
@endsection